...
func _test_sparse_paint_undo_redo_and_persistence() -> void:
	...
	_check(region.biome_override_id(center) == 3, "material undo changed biome channel")
	...
	_check(region.material_override_id(center) == 4, "biome undo changed material channel")
	_check(region.material_override_strength(center) == 255, "biome undo changed material strength")
	...
	_check(region.material_override_id(center) == 4, "biome redo changed material channel")
	_check(region.material_override_strength(center) == 255, "biome redo changed material strength")
...
func _test_override_texture_encoding_and_erase() -> void:
	...
	var texture: ImageTexture = sync.texture()
	_check(texture != null, "override texture is missing")
	_check(texture.get_width() == 65 and texture.get_height() == 65, "override texture resolution is incorrect")
	_check(sync.last_upload_bytes() == 65 * 65 * 4, "override upload accounting is incorrect")
	var sample: Color = sync.sample_override_at_world(biome_command.center_world)
	_check(absf(sample.r - 6.0 / 7.0) < 0.02, "override backing image biome id encoding is incorrect")
	_check(sample.g > 0.95, "override backing image biome strength is incorrect")
	_check(sample.b > 0.95, "override backing image material id encoding is incorrect")
	_check(sample.a > 0.95, "override backing image material strength is incorrect")
	...
	sync.flush_all()
	sample = sync.sample_override_at_world(biome_command.center_world)
